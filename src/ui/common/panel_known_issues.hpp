...
inline void render_panel_known_issues() {
    ...
    if (ImGui::CollapsingHeader("1. Gear Chance-on-Hit / Proc Special Effects (Backlog)", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (ImGui::CollapsingHeader("2. Spell Downranking & Base Spell Data", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (ImGui::CollapsingHeader("3. Encounter Mechanics & Target Scope", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (ImGui::CollapsingHeader("4. Pet Survival & Encounter Uptime", ImGuiTreeNodeFlags_DefaultOpen)) {
