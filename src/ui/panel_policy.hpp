...
inline void render_priority_chain_subpane(const std::vector<PriorityRule>& rules)
{
  ...
  if (ImGui::BeginChild("PriorityRuleChainBox",
                        ImVec2(0, 0),
    ...
    for (size_t i = 0; i < rules.size(); ++i)
    {
      ...
      if (rule.action == PriorityAction::LIFE_TAP)
      {
        ...
        border_col = IM_COL32(230, 76, 76, 230);
               ...
               rule.action == PriorityAction::DECIMATION_SOUL_FIRE)
      {
      ...
      else if (rule.action == PriorityAction::RACIAL_EUREKA || rule.action == PriorityAction::RACIAL_BLOOD_FURY ||
               rule.action == PriorityAction::RACIAL_BERSERKING)
      {
      ...
      if (is_hovered)
      {
        ...
        ImGui::Separator();
        ImGui::TextDisabled(
            "Base Spell: %s | Condition: %s", spell_id_to_name(rule.spell_id), rule.condition_summary.c_str());
        ImGui::EndTooltip();
      }
