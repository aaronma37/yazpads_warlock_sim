...
inline void render_panel_theorycrafting(const WarlockSimulator& sim)
{
          ...
          ImGuiTreeNodeFlags_DefaultOpen))
  {
    ...
    ImGui::TextWrapped(
        "Let T_rem be the remaining fight duration. For all Spell Power SP >= 0 and all shadow multipliers M_shadow > "
        "0, "
        "the hybrid policy P*(T_rem >= 60s -> Bane of Doom, T_rem < 60s -> Bane of Agony) strictly dominates "
        "the static policy P_Agony (always cast Bane of Agony) in Total Damage, Cast-Time Efficiency (DPS/GCD), and "
        "Mana Efficiency.");

    ImGui::Spacing();

    // Lemma 1: Direct Damage Bound & Imp BoA
    ImGui::TextColored(ImVec4(0.85f, 0.85f, 0.95f, 1.0f),
                       "- Lemma 1 (Direct Spell Formulas with 2/2 Improved Bane of Agony):");
    ImGui::Indent(12.0f);
    ImGui::TextWrapped("Over any 60.0-second window, Bane of Doom completes 1 full cycle (1 tick at t=60s), while Bane "
                       "of Agony requires "
                       "2.5 full 24.0-second cycles (2.5 casts = 30 ticks).");
    ImGui::TextColored(ImVec4(0.85f, 0.85f, 0.85f, 1.0f), "  - D_Doom_direct(SP) = (1742.0 + 4.00 * SP) * M_shadow");
    ImGui::TextWrapped("  - In WoW Forever, Improved Bane of Agony (2/2) increases total damage by +10%% (amplifying "
    ...
    ImGui::Spacing();
    // Lemma 2: Talent Invariance & Edge Cases
    ImGui::TextColored(ImVec4(0.85f, 0.85f, 0.95f, 1.0f),
                       "- Lemma 2 (Invariance Across All Other Talent Trees & Procs):");
    ImGui::Indent(12.0f);
    ImGui::TextWrapped(
        "- Shadow Multipliers (Shadow Mastery 5/5, Demonic Sacrifice, ISB): Both spells are 100%% Shadow school; all "
        "percentage multipliers amplify both identically (M_shadow > 0 preserves inequalities).");
    ImGui::TextWrapped("- DoT Critical Strikes (Pandemic 3/3): Both Agony and Doom ticks share identical crit chance "
                       "and identical Pandemic bonus multipliers (1.0 + 0.5 * (1 + Pandemic/3)).");
    ImGui::TextWrapped("- Amplify Curse (3-min CD): +50%% damage to 1 Agony cast (+303.6 + 0.88*SP). Over a full 180s "
                       "cycle, this adds only +101.2 + 0.293*SP per minute, which is heavily outweighed by Doom's "
                       "freed filler budget.");
    ImGui::Unindent(12.0f);

    ImGui::Spacing();
    // Lemma 3: GCD & Casting Budget Opportunity Cost
    ImGui::TextColored(ImVec4(0.85f, 0.85f, 0.95f, 1.0f), "- Lemma 3 (GCD & Casting Budget Opportunity Cost):");
    ImGui::Indent(12.0f);
    ImGui::TextWrapped("Casting Bane of Doom requires 1 GCD (1.50s) per 60.0s window. Casting Bane of Agony requires "
                       "2.5 GCDs (3.75s) per 60.0s window.");
    ImGui::TextWrapped(
        "By casting Doom instead of Agony, the warlock reclaims dt = 3.75s - 1.50s = 2.25s of casting time per 60s.");
    ImGui::TextWrapped("With 5/5 Bane (2.5s Shadow Bolt cast time), this 2.25s is converted into 2.25 / 2.50 = 0.90 "
                       "extra Shadow Bolt casts (or 0.75 without Bane):");
    ImGui::TextColored(
        ImVec4(0.85f, 0.85f, 0.85f, 1.0f),
        "  - dD_filler(SP) [5/5 Bane] = 0.90 * (510.0 + 0.85714 * SP) * M_shadow = (459.0 + 0.7714 * SP) * M_shadow");
    ImGui::TextColored(
        ImVec4(0.85f, 0.85f, 0.85f, 1.0f),
        "  - dD_filler(SP) [No Bane]  = 0.75 * (510.0 + 0.85714 * SP) * M_shadow = (382.5 + 0.6429 * SP) * M_shadow");
    ImGui::Unindent(12.0f);

    ImGui::Spacing();
    // Lemma 4: Total Net Yield
    ImGui::TextColored(ImVec4(0.85f, 0.85f, 0.95f, 1.0f), "- Lemma 4 (Combined Net Yield & Strict Dominance):");
    ImGui::Indent(12.0f);
    ...
    ImGui::Spacing();
    // Lemma 5: Boundary Condition
    ImGui::TextColored(ImVec4(0.85f, 0.85f, 0.95f, 1.0f), "- Lemma 5 (Boundary Condition for T_rem < 60s):");
    ImGui::Indent(12.0f);
    ImGui::TextWrapped("Because Bane of Doom inflicts all damage exactly at t=60.0s, its yield is 0 if the encounter "
    ...
    if (ImPlot::BeginPlot("Curse of Doom vs Curse of Agony (60s Window)", ImVec2(-1, 290)))
    {
      ...
      ImPlot::PlotLine(
          "Net Advantage Delta [5/5 Bane] (Slope: +0.371)", sp_vals.data(), net_delta_bane_curve.data(), plot_points);
      ImPlot::PlotLine(
          "Net Advantage Delta [No Bane] (Slope: +0.243)", sp_vals.data(), net_delta_nobane_curve.data(), plot_points);
      ImPlot::EndPlot();
    }
          ...
          ImGuiTreeNodeFlags_DefaultOpen))
  {
    ...
    ImGui::TextWrapped(
        "For all Spell Power SP >= 0, encounter durations T > 0, and rotation policies, investing 1 talent point into "
        "Amplify Curse (1/1) strictly yields greater marginal damage than investing 1 or 2 talent points into Improved "
        "Bane of Agony (1/2 or 2/2).");

    ImGui::Spacing();

    // Lemma 1: Under Optimal Policy P*
    ImGui::TextColored(ImVec4(0.85f, 0.85f, 0.95f, 1.0f),
                       "- Lemma 1 (Marginal Yield Under Optimal Policy P* [Doom on T >= 60s]):");
    ImGui::Indent(12.0f);
    ImGui::TextWrapped(
        "Under the optimal policy P*, Bane of Doom is cast for all 60.0s windows. Bane of Agony is cast only during "
        "the terminal encounter tail (T_rem < 60.0s), resulting in N_Agony in {0, 1, 2} casts per encounter (average "
        "~= 1 cast).");
    ...
    ImGui::TextColored(
        ImVec4(0.85f, 0.85f, 0.85f, 1.0f),
        "  - Gain from Imp Bane of Agony (2 pts = +10%%): dD_Imp2 = N_Agony * 0.10 * D_Agony(SP) = +0.100 * D_Agony");
    ImGui::TextColored(ImVec4(0.4f, 1.0f, 0.4f, 1.0f),
    ...
    ImGui::Spacing();
    // Lemma 2: Under 100% Continuous Agony Spam
    ImGui::TextColored(ImVec4(0.85f, 0.85f, 0.95f, 1.0f),
                       "- Lemma 2 (Marginal Yield Under 100%% Continuous Agony Uptime [No Doom]):");
    ImGui::Indent(12.0f);
    ImGui::TextWrapped(
        "Even under sub-optimal 100%% continuous Agony uptime (refreshing every 24.0s), over one 180.0s Amplify Curse "
    ...
    ImGui::Spacing();
    // Lemma 3: Action Economy & Tree Progression
    ImGui::TextColored(ImVec4(0.85f, 0.85f, 0.95f, 1.0f), "- Lemma 3 (Action Economy & Tree Progression):");
    ImGui::Indent(12.0f);
    ImGui::TextWrapped(
        "- Instant & Off-GCD: Amplify Curse incurs 0.0s cast time and 0.0s GCD, weaving with zero opportunity cost.");
  ...
  // -------------------------------------------------------------------------------------------------
  if (ImGui::CollapsingHeader("[Unverified] Untalented Shadow Bolt (No Talents) vs Maximized Channeled Wrack (6.0s Budget)",
                              ImGuiTreeNodeFlags_DefaultOpen))
  {
    ImGui::Indent(8.0f);

    ImGui::TextWrapped(
        "Comparing the worst possible version of Shadow Bolt (0/5 Bane, 3.0s hardcast, zero talents, no ISB, no Ruin) "
        "against the best possible version of 6.0s Channeled Wrack (11 Affliction talent points, 3 active DoTs with "
        "+10%% amplification, and 2/2 Nightfall procs).");

    ImGui::Spacing();

    // Lemma 1: Worst-Case Shadow Bolt Yield in 6.0s
    ImGui::TextColored(ImVec4(0.85f, 0.85f, 0.95f, 1.0f),
                       "- Lemma 1 (Worst-Case Untalented Shadow Bolt in 6.0s Budget):");
    ImGui::Indent(12.0f);
    ImGui::TextWrapped("With 0/5 Bane, Shadow Bolt has a 3.0s base cast time. In a 6.0s window, the warlock hardcasts "
    ...
    ImGui::Spacing();
    // Lemma 2: Best-Case Channeled Wrack Yield with Pandemic & Malevolence
    ImGui::TextColored(ImVec4(0.85f, 0.85f, 0.95f, 1.0f),
                       "- Lemma 2 (Best-Case Maximized Channeled Wrack in 6.0s with Crits):");
    ImGui::Indent(12.0f);
    ImGui::TextWrapped("Wrack delivers 6 ticks (1 tick/sec) over a 6.0s channel. Fully maxing Affliction talents (3/3 "
                       "Imp Drains +20%%, "
                       "3/3 Soul Siphon +36%%, 5/5 Malediction +5%%, 5/5 Malevolence +5%% crit, and 3/3 Pandemic "
                       "+100%% DoT crit bonus):");
    ImGui::TextColored(
        ImVec4(0.85f, 0.85f, 0.85f, 1.0f),
        "  - Direct Wrack: D_direct = (212.0 + 1.00 * SP) * 1.20 * 1.36 * 1.05 = (363.28 + 1.7136 * SP) * M_shadow");
    ImGui::TextColored(
        ImVec4(0.85f, 0.85f, 0.85f, 1.0f),
    ...
    ImGui::TextWrapped("  - For Untalented SB (no Ruin, 1.5x crit at c=15%%): M_crit_sb = 1 + 0.50 * 0.15 = 1.075x ==> "
                       "D_SB_worst = (1096.50 + 1.8429 * SP) * M_shadow");
    ImGui::Unindent(12.0f);

    ImGui::Spacing();
    // Lemma 3: Exact Inflection / Crossover Point
    ImGui::TextColored(ImVec4(0.85f, 0.85f, 0.95f, 1.0f), "- Lemma 3 (Inflection / Crossover Threshold SP*):");
    ImGui::Indent(12.0f);
    ImGui::TextWrapped("Setting D_Wrack_best(SP) = D_SB_worst(SP) yields the exact mathematical crossover threshold:");
    ...
    ImGui::TextWrapped("- For SP < 1561 SP: Untalented Shadow Bolt strictly produces more damage (+552 dmg at 0 SP, "
                       "+234 dmg at 900 SP).");
    ImGui::TextWrapped(
        "- For SP >= 1561 SP: Maximized Wrack overtakes untalented Shadow Bolt due to 3/3 Pandemic 2.0x DoT crits.");
    ...
    ImGui::TextColored(
        cur_sb_delta >= 0 ? ImVec4(0.3f, 1.0f, 0.4f, 1.0f) : ImVec4(1.0f, 0.4f, 0.4f, 1.0f), "%+.0f dmg", cur_sb_delta);
