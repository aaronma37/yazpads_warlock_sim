...
{
  std::string aff = encode_tree_url(FOREVER_AFFLICTION_NODES, [&](size_t i) { return t.aff.get_points_by_index(i); });
  std::string demo = encode_tree_url(FOREVER_DEMONOLOGY_NODES, [&](size_t i) { return t.demo.get_points_by_index(i); });
  std::string destro =
      encode_tree_url(FOREVER_DESTRUCTION_NODES, [&](size_t i) { return t.destro.get_points_by_index(i); });
...
template <size_t N>
inline void render_tree_column(const char* tree_name,
    ...
    for (int col = 1; col <= 4; ++col)
    {
      ...
      if (ImGui::IsItemHovered())
      {
        ...
        if (!tier_unlocked)
        {
          ImGui::Spacing();
          ImGui::TextColored(
              ImVec4(1.0f, 0.3f, 0.3f, 1.0f), "Requires %d points in %s Talents", (row - 1) * 5, tree_name);
        }
...
inline void render_panel_talents(WarlockSimulator& sim)
{
  ...
  ImGui::SameLine();
  {
    ...
    const char* preview = (preset_idx >= 0 && preset_idx < (int)presets.size()) ? presets[preset_idx].display_name
                                                                                : "-- Select a preset --";
